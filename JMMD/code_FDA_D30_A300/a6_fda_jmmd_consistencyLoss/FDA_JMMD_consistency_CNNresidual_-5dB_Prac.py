...
from JMMD.helper.utils_GAN import post_val, train_step_cnn_residual_fda_jmmd_rawThenScale_domainAware, val_step_cnn_residual_fda_jmmd_rawThenScale_domainAware
...
for sub_folder in sub_folder_:
    ...
    for epoch in range(n_epochs):
        ...
        ##########################
        # 
        train_step_output = train_step_cnn_residual_fda_jmmd_rawThenScale_domainAware(model, loader_H, loss_fn, optimizer, lower_range=-1, 
        ...
        if (epoch==epoch_min) or (epoch+1>epoch_min and (epoch-epoch_min)%epoch_step==0) and epoch!=n_epochs-1:
            # 
            H_sample, epoc_val_return = val_step_cnn_residual_fda_jmmd_rawThenScale_domainAware(model, loader_H_eval, loss_fn, lower_range, 
            ...
            _, epoc_val_return, H_val_gen = val_step_cnn_residual_fda_jmmd_rawThenScale_domainAware(model, loader_H_eval, loss_fn, lower_range, 
                                            weights=weights, linear_interp=linear_interp, return_H_gen=True)
        else:
            # 
            _, epoc_val_return = val_step_cnn_residual_fda_jmmd_rawThenScale_domainAware(model, loader_H_eval, loss_fn, lower_range, 
                                        weights=weights, linear_interp=linear_interp)
