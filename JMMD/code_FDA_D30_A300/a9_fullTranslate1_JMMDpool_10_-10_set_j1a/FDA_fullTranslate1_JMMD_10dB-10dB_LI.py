...
print("JMMD, reconstruction training first then domain adaptation training, domain weight 0.1e-3 → 7.5e-3, est weight 1.5 → 0.8, warmup epochs 80")
...
from JMMD.helper.utils_GAN import post_val, train_step_cnn_residual_FDAfullTranslation1_jmmd, val_step_cnn_residual_jmmd
...
for sub_folder in sub_folder_:
    ...
    for epoch in range(n_epochs):
        ...
        ##########################
        # 
        train_step_output = train_step_cnn_residual_FDAfullTranslation1_jmmd(model, loader_H, loss_fn, optimizer, lower_range=-1, 
                        save_features=False, weights=weights, linear_interp=linear_interp,
                        fda_win_h=fda_win_h, fda_win_w=fda_win_w, fda_weight=fda_weight, jmmd_loss_fn=jmmd_loss)
