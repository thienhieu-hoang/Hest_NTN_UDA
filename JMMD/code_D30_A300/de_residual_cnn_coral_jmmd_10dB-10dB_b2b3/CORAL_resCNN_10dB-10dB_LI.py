...
from JMMD.helper.utils_GAN import post_val, train_step_cnn_residual_coral_jmmd, val_step_cnn_residual_coral_jmmd
...
for sub_folder in sub_folder_:
    ...
    print("4 blocks, CORAL, no FDA, extract layers = blocks 2, 3")
    # 
    ...
    for epoch in range(n_epochs):
        ...
        ##########################
        # 
        train_step_output = train_step_cnn_residual_coral_jmmd(model, loader_H, loss_fn, optimizer, lower_range=-1, 
                        coral_loss_fn=coral_loss_fn, save_features=False, weights=weights, linear_interp=linear_interp)
        ...
        if (epoch==epoch_min) or (epoch+1>epoch_min and (epoch-epoch_min)%epoch_step==0) and epoch!=n_epochs-1:
            # 
            H_sample, epoc_val_return = val_step_cnn_residual_coral_jmmd(model, loader_H_eval, loss_fn, lower_range, 
                                            coral_loss_fn=coral_loss_fn, weights=weights, linear_interp=linear_interp)
            ...
            _, epoc_val_return, H_val_gen = val_step_cnn_residual_coral_jmmd(model, loader_H_eval, loss_fn, lower_range, 
                                            coral_loss_fn=coral_loss_fn, weights=weights, linear_interp=linear_interp, return_H_gen=True)    
        else:
            # 
            _, epoc_val_return = val_step_cnn_residual_coral_jmmd(model, loader_H_eval, loss_fn, lower_range, 
                                        coral_loss_fn=coral_loss_fn, weights=weights, linear_interp=linear_interp)
