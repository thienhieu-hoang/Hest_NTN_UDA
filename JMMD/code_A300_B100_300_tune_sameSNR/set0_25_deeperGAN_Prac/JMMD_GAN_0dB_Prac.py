...
source_data_file_path = os.path.abspath(os.path.join(code_dir, '..', '..', '..', 'generatedChan', 'MATLAB', 'TDL_A_300_sim', f'SNR_{SNR}dB', 'matlabNTN.mat'))
...
weights = {
    ...
    'domain_weight': 2.5,       # JMMD loss weight (domain adaptation)
...
# Paths to save
path_temp = project_root + f'/JMMD/model/GAN_cal_D30_10_B100_300_tune/test_deep_set0_25/'
os.makedirs(os.path.dirname(path_temp), exist_ok=True)
idx_save_path = loader.find_incremental_filename(path_temp,'ver', '_', '')

save_model = False
model_path = project_root + f'/JMMD/model/GAN_cal_D30_10_B100_300_tune/test_deep_set0_25/ver' + str(idx_save_path) + '_'
...
from JMMD.helper.utils_GAN import GAN, DeeperGAN
...
for sub_folder in sub_folder_:
    ...
    # 
    model = DeeperGAN(gen_l2=None, disc_l2=1e-5)  # l2 regularization for generator and discriminator
    # 
...
for epoch in range(n_epochs):
    ...
    if epoch==0 or epoch == n_epochs-1:
        # save_features == save features to .h5 files to calculate PAD
        save_features = True
    ...
    train_step_output = train_step_wgan_gp_jmmd(model, loader_H, loss_fn, optimizer, lower_range=-1, 
                    save_features=save_features, weights=weights, linear_interp=linear_interp)
    ...
    if save_features and (weights['domain_weight']!=0):
