...
weights = {
    # Core loss weights
    'adv_weight': 0.02,         # GAN adversarial loss weight
    'est_weight': 0.8,          # Estimation loss weight (main task)
    'domain_weight': 1.0,       # JMMD loss weight (domain adaptation)
...
# Paths to save
path_temp = project_root + f'/JMMD/model/GAN_cal_A300_B100_300_tune_sameSNR/set2/'
os.makedirs(os.path.dirname(path_temp), exist_ok=True)
idx_save_path = loader.find_incremental_filename(path_temp,'ver', '_', '')

save_model = False
model_path = project_root + f'/JMMD/model/GAN_cal_A300_B100_300_tune_sameSNR/set2/ver' + str(idx_save_path) + '_'
