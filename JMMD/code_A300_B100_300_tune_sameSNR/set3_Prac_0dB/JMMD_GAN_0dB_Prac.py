...
weights = {
    # Core loss weights
    'adv_weight': 0.08,         # GAN adversarial loss weight
    'est_weight': 0.6,          # Estimation loss weight (main task)
    'domain_weight': 1.8,       # JMMD loss weight (domain adaptation)
...
# Paths to save
path_temp = project_root + f'/JMMD/model/GAN_cal_A300_B100_300_tune_sameSNR/set3/'
os.makedirs(os.path.dirname(path_temp), exist_ok=True)
idx_save_path = loader.find_incremental_filename(path_temp,'ver', '_', '')

save_model = False
model_path = project_root + f'/JMMD/model/GAN_cal_A300_B100_300_tune_sameSNR/set3/ver' + str(idx_save_path) + '_'
