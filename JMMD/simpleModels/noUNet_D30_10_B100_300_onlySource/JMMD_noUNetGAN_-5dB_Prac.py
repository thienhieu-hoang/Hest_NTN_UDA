...
# Paths to save
path_temp = project_root + f'/JMMD/model/noNetGAN_onlySource_D30_B100_300/{SNR}_dB/'
os.makedirs(os.path.dirname(path_temp), exist_ok=True)
idx_save_path = loader.find_incremental_filename(path_temp,'ver', '_', '')

save_model = False
model_path = project_root + f'/JMMD/model/noNetGAN_onlySource_D30_B100_300/{SNR}_dB/ver' + str(idx_save_path) + '_'
...
from JMMD.helper.utils_GAN import GAN, SimplePix2PixGenerator
...
for sub_folder in sub_folder_:
    ...
    # 
    model = GAN(n_subc=312, generator= SimplePix2PixGenerator, gen_l2=None, disc_l2=1e-5, extract_layers=['d3','d4'])  # l2 regularization for generator and discriminator
    # 
    gen_optimizer = tf.keras.optimizers.Adam(learning_rate=lr_gen, beta_1=0.5, beta_2=0.9)
    disc_optimizer = tf.keras.optimizers.Adam(learning_rate=lr_disc, beta_1=0.5, beta_2=0.9)  # WGAN-GP uses Adam optimizer with beta_1=0.5
    # 
    ####
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch==epoch_min) or (epoch+1>epoch_min and (epoch-epoch_min)%epoch_step==0) and epoch!=n_epochs-1:
            ...
            _, epoc_val_return, H_val_gen = val_step_wgan_gp_source_only(model, loader_H_eval, loss_fn, lower_range, 
