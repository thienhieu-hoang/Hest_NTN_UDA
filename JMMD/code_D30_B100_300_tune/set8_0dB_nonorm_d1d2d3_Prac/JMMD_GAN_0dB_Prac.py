...
weights = {
    ...
    'domain_weight': 3.0,       # JMMD loss weight (domain adaptation)
...
# Paths to save
path_temp = project_root + f'/JMMD/model/GAN_cal_D30_10_B100_300_tune/set8_d1d2d3/'
os.makedirs(os.path.dirname(path_temp), exist_ok=True)
idx_save_path = loader.find_incremental_filename(path_temp,'ver', '_', '')

save_model = False
model_path = project_root + f'/JMMD/model/GAN_cal_D30_10_B100_300_tune/set8_d1d2d3/ver' + str(idx_save_path) + '_'
...
for sub_folder in sub_folder_:
    ...
    # 
    model = GAN(n_subc=312, gen_l2=None, disc_l2=1e-5, extract_layers=['d1','d2','d3'])  # l2 regularization for generator and discriminator
    # 
