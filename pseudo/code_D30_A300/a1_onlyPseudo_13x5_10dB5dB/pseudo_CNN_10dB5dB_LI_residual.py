...
from pseudo.helper.utils_pseudo import save_checkpoint_pseudo as save_checkpoint
...
pseudo_data_file_path = os.path.abspath(os.path.join(code_dir, '..', '..', '..', 'generatedChan', 'MATLAB', 'pseudoChannelD30_A300', f'SNR_{SNR}dB_13x5', 'matlabNTN.mat'))
...
scheduler = WeightScheduler(strategy='reconstruction_first', start_domain_weight=0.0, end_domain_weight=0.0,
                            start_est_weight=1.0, end_est_weight=1.0, warmup_epochs=80) 
...
# ============ Pseudo data ==============
pseudo_file = h5py.File(pseudo_data_file_path, 'r')
H_true_pseudo = pseudo_file['H_perfect']
N_samp_pseudo = H_true_pseudo.shape[0]
print('N_samp_pseudo = ', N_samp_pseudo)
...
indices_pseudo = np.arange(N_samp_pseudo)
np.random.shuffle(indices_pseudo)
...
N_samp = max(N_samp_source, N_samp_target, N_samp_pseudo) 
indices_source = np.resize(indices_source, N_samp)
indices_target = np.resize(indices_target, N_samp)
indices_pseudo = np.resize(indices_pseudo, N_samp)
...
indices_train_pseudo = indices_pseudo[:train_size]
indices_val_pseudo   = indices_pseudo[train_size:train_size + val_size]
...
# indices_train_pseudo = indices_pseudo[:96]
# indices_val_pseudo = indices_pseudo[2032:]

print('train_size = ', indices_train_source.shape[0])
print('val_size = ', indices_val_source.shape[0])

class DataLoaders:
    def __init__(self, file, indices_train, indices_val, tag='li', batch_size=32): 
...
# Pseudo domain
class_dict_pseudo = {
    'GAN_linear': DataLoaders(pseudo_file, indices_train_pseudo, indices_val_pseudo, tag='li', batch_size=batch_size),
}
...
from pseudo.helper.utils_pseudo import train_step_cnn_residual_pseudo, val_step_cnn_residual_pseudo
...
for sub_folder in sub_folder_:
    ...
    loader_H_true_train_pseudo = class_dict_pseudo[sub_folder].true_train
    loader_H_input_train_pseudo = class_dict_pseudo[sub_folder].input_train
    loader_H_true_val_pseudo = class_dict_pseudo[sub_folder].true_val
    loader_H_input_val_pseudo = class_dict_pseudo[sub_folder].input_val
    ...
    val_metrics = {
        ...
        'val_est_loss_pseudo': [],      # pseudo estimation loss 
        'val_est_loss_source': [],      # source estimation loss (monitoring)
        'val_est_loss_target': [],      # target estimation loss (monitoring)
    ...
    # 
    # from JMMD.helper.utils_GAN import MemoryEfficientCORALLoss
    ...
    for epoch in range(n_epochs):
        ...
        #
        loader_H_true_train_pseudo.reset()
        loader_H_input_train_pseudo.reset()        
        ...
        ##########################
        # 
        train_step_output = train_step_cnn_residual_pseudo(model, loader_H, loss_fn, optimizer, lower_range=-1, 
        ...
        if (epoch==epoch_min) or (epoch+1>epoch_min and (epoch-epoch_min)%epoch_step==0) and epoch!=n_epochs-1:
            # 
            H_sample, epoc_val_return = val_step_cnn_residual_pseudo(model, loader_H_eval, loss_fn, lower_range, 
            ...
            _, epoc_val_return, H_val_gen = val_step_cnn_residual_pseudo(model, loader_H_eval, loss_fn, lower_range, 
                                            weights=weights, linear_interp=linear_interp, return_H_gen=True)    
        else:
            # 
            _, epoc_val_return = val_step_cnn_residual_pseudo(model, loader_H_eval, loss_fn, lower_range, 
                                        weights=weights, linear_interp=linear_interp)
