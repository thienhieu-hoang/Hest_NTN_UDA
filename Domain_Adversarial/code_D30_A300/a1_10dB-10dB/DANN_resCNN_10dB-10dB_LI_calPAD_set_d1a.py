...
scheduler = WeightScheduler(strategy='reconstruction_first', start_domain_weight=0.01, end_domain_weight=0.05,
                            # start_adv_weight=0.005, end_adv_weight=0.005,
                            start_adv_weight=0.005, end_adv_weight=0.1,
                            start_est_weight=1.5, end_est_weight=0.8, warmup_epochs=80) 
                            # adv_weight   # forces CNN to make target features look like source features
                            # domain_weight   # disc model to discriminate the 2 features
...
for sub_folder in sub_folder_:
    ...
    train_metrics = {
        ...
        'train_adv_loss': [],       # adversarial loss   # forces CNN to make target features look like source features
        'train_domain_loss': [],    # domain disc loss   # disc model to discriminate the 2 features
    ...
    val_metrics = {
        ...
        'val_adv_loss': [],             # How well CNN fools discriminator
        'val_domain_loss': [],          # domain disc loss
    ...
    print("4 blocks, DANN, no FDA, extract layers = block 2")
    ...
    # 
    from JMMD.helper.utils_GAN import GlobalPoolingCORALLoss
    ...
    for epoch in range(n_epochs):
        ...
        ##########################
        # 
        train_step_output = train_step_cnn_residual_dann(model, domain_disc, loader_H, loss_fn, optimizers, lower_range=-1, 
        ...
        train_epoc_loss_adv        = train_step_output.avg_epoc_loss_adv         # adversarial loss to make target features look like source features
        ...
        if return_features and (weights['domain_weight']!=0) and (epoch == n_epochs-1):
        ...
        train_metrics['train_adv_loss'].append(train_epoc_loss_adv)
        print(f"epoch {epoch+1}/{n_epochs} Average Adversarial Loss (in Source domain): {train_epoc_loss_adv:.6f}")
        
        train_metrics['train_domain_loss'].append(train_epoc_loss_domain)
        print(f"epoch {epoch+1}/{n_epochs} Average DANN Loss: {train_epoc_loss_domain:.6f}")  # Updated print message
        ...
        if (epoch==epoch_min) or (epoch+1>epoch_min and (epoch-epoch_min)%epoch_step==0) and epoch!=n_epochs-1:
            # 
            H_sample, epoc_val_return = val_step_cnn_residual_dann(model, domain_disc, loader_H_eval, loss_fn, lower_range, 
            ...
            _, epoc_val_return, H_val_gen = val_step_cnn_residual_dann(model, domain_disc, loader_H_eval, loss_fn, lower_range, 
                                            weights=weights, linear_interp=linear_interp, return_H_gen=True)    
        else:
            # 
            _, epoc_val_return = val_step_cnn_residual_dann(model, domain_disc, loader_H_eval, loss_fn, lower_range, 
                                        weights=weights, linear_interp=linear_interp)
