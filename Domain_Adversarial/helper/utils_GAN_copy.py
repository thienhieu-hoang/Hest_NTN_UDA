...
def save_checkpoint(model, save_model, model_path, sub_folder, epoch, figLoss, savemat, train_loss, train_est_loss, train_domain_loss, train_est_loss_target,
    ...
    if save_model:
        ...
        # Save checkpoint using write() method for consistency
        checkpoint_path = f"{checkpoint_dir}/epoch_{epoch+1}"
        milestone_path = ckpt.write(checkpoint_path)
        ...
        try:
            ...
            print(f"⚠️  Warning: Could not save optimizer configs: {e}")
...
def load_checkpoint(model, model_path, sub_folder, epoch_load, optimizer=None, domain_model=None, domain_weight=True):
    ...
    Args:
        ...
        epoch_load: Epoch number to load (0-indexed)
    ...
    """
    checkpoint_dir = f"{model_path}/{sub_folder}/model"
    # Try different possible checkpoint path formats
    possible_paths = [
        f"{checkpoint_dir}/epoch_{epoch_load+1}",
        f"{checkpoint_dir}/epoch_{epoch_load+1}-1"
    ]
    
    checkpoint_path = None
    # Check if any of the direct paths exist
    for path in possible_paths:
        if path and os.path.exists(path + ".index"):  # TensorFlow checkpoints have .index files
            checkpoint_path = path
            break
    
    # If no direct path found, try latest checkpoint
    ...
    if checkpoint_path is None:
        raise FileNotFoundError(f"No checkpoint found for epoch {epoch_load+1} in {checkpoint_dir}")
    ...
    if optimizer is not None:
        ...
        if not os.path.exists(config_path):
            raise FileNotFoundError(f"Optimizer config file not found: {config_path}")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load optimizer configs: {e}")
    ...
    try:
        ...
        status.expect_partial()  # Use expect_partial() to handle cases where not all variables are restored
        print(f"✅ Checkpoint restored successfully: {checkpoint_path}")
    except Exception as e:
        raise RuntimeError(f"Failed to restore checkpoint {checkpoint_path}: {e}")
